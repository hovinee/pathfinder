...
import { thumbnailDiagnosis } from '@/data/unity/data'
...
const DignosisModal = ({ setSelectPlace }: TProps) => {
        ...
        {selectHealing && (
          <div className="flex h-full gap-[1.5rem]">
            <div className="flex flex-col">
              <AutoSizeImage
                src={
                  thumbnailDiagnosis[healingNum].thumbnail_image[thumbnailNum]
                }
                className="h-[45rem] w-full"
                rounded="10"
              />
              <div className="mt-[1.5rem] grid flex-1 grid-cols-4 gap-[1.5rem]">
                {thumbnailDiagnosis[healingNum].thumbnail_image.map(
                  (image, index) => (
                    <>
                      {thumbnailNum !== index && (
                        <AutoSizeImage
            ...
            <div className="flex h-full w-[47.5rem] flex-col rounded-[1rem] bg-white p-[3rem]">
              <div>
                <CSText size="31" color="black" weight="bold">
                  {thumbnailDiagnosis[healingNum].title}
                </CSText>
                <CSText
                  size="16"
                  color="black"
                ...
                >
                  {thumbnailDiagnosis[healingNum].description}
                </CSText>
                <Audio audioPath={thumbnailDiagnosis[healingNum].audio} black />
                <div className="mt-[0.5rem]">
                  {thumbnailDiagnosis[healingNum].tag.map((value, index) => (
                    <div
                ...
                <div className="mt-[2rem] flex gap-[2.3rem]">
                  <CSButton
                    ...
                    weight="bold"
                    onClick={() =>
                      thumbnailDiagnosis[healingNum].path
                        ? window.open(thumbnailDiagnosis[healingNum].path!)
                        : alert('준비중입니다')
                    }
                  >
                    입장하기
